(function () {
  ...
  const { ensureWorkerBundle, ensureWorkerPolyfillBundle } =
    webhid.import("bgBundle");
  ...
  async function migrateLegacyStorage() {
    ...
    for (const [key, value] of Object.entries(all)) {
      ...
      for (const deviceId of value)
        store.put({ origin, deviceId: Number(deviceId) });
      await txDone(tx);
      keysToRemove.push(key);
    }
    ...
    for (const [key, value] of Object.entries(all)) {
      ...
      for (const [name, v] of Object.entries(value))
        patch[siteSettingKey(origin, name)] = v;
      keysToRemove.push(key);
    }
    ...
    for (const dev of devices) {
      if (dev && typeof dev.collections === "string") {
        try {
          ...
          logger.warn(
            "decodeCollectionsTlv failed for device",
            dev.deviceId,
            e.message,
          );
          dev.collections = [];
  ...
  async function refreshWorkerPolyfillSites() {
    ...
    for (const [key, val] of Object.entries(all)) {
      ...
      if (
        parsed &&
        parsed.scope === "site" &&
        parsed.name === "workerPolyfillEnabled" &&
        val
      ) {
    ...
    notificationsApi.onClicked.addListener(function () {
      if (pendingPicker.size > 0) {
        ...
        if (browser.pageAction.openPopup)
          browser.pageAction
            .openPopup()
            .catch((e) => logger.debug("openPopup failed", e));
        notificationsApi
          .clear("webhid-picker")
