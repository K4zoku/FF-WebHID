...
- **Chromium testbed** (`chromium-testbed` branch): runs the addon itself on Chromium via `TARGET=chromium npm run build:addon` (unpacked `dist/chromium/`) to measure addon overhead vs native WebHID in the same runtime. Conventions:
  ...
  - The default `devicePickerMode` `'modal'` (in-page closed-shadow-DOM picker) is used, driven with the keyboard like the Firefox e2e helper. `'window'` was rejected: in the testbed it lists every device the daemon sees (real hardware included), so selection is non-deterministic, while the modal honors the requestDevice filter and lists only the mock.
  - Chromium does not render SVG extension icons; `addon/icons/*.png` are rasterized from the SVGs.
  - Chromium extension messaging historically serializes with JSON (`JSON.stringify`), turning TypedArrays into plain keyed objects without `length`, so `normalizeBytes` (nm.js for sends, bridge.js for input/feature data) had to rebuild the Uint8Array at the boundary. `manifest.chromium.json` sets `"message_serialization": "structured_clone"` (Chrome 148+, feature flag in this range), so TypedArrays now survive the hop and `normalizeBytes` is a passthrough on Chromium; the `instanceof Uint8Array` passthrough keeps Firefox zero-copy either way, and the rebuild branches stay as fallback for JSON-serializing browsers (Safari) or older Chrome. Note native messaging is always JSON regardless of the key.
