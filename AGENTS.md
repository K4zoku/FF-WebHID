...
- **Chromium testbed** (`chromium-testbed` branch): runs the addon itself on Chromium via `TARGET=chromium npm run build:addon` (unpacked `dist/chromium/`) to measure addon overhead vs native WebHID in the same runtime. Conventions:
  ...
  - Chromium MV3 cannot share `globalThis` across multi-file script arrays (one SW file, content-script arrays), so the build bundles each world (background, MAIN, ISOLATED) into one file from the `addon/manifest.json` lists, prepending `js/utils/browser-shim.js` (`browser = chrome`). The shim is also injected into extension page HTMLs (they call `browser.*` and Chromium page realms have no `browser`). Firefox is unaffected (shim is a no-op; the chromium target skips web-ext).
  - `registerWebRequestHandlers` early-returns when `browser.runtime.getURL('')` starts with `chrome-extension://`: MV3 webRequest is observational only. The gate only disables the shadow-URL spawn; the benchmark page has no CSP, so ws/wt still spawn their worker via the blob path (`workerSpawnMode: 'blob'`). Without the gate the blocking listener registration throws and breaks boot.
  ...
  - Chromium serializes TypedArrays in extension messaging (content script ↔ SW, both directions) into plain keyed objects without `length`. `normalizeBytes` (nm.js for sends, bridge.js for input/feature data) rebuilds the Uint8Array at the boundary; the `instanceof Uint8Array` passthrough keeps Firefox zero-copy.
