...
test.describe('HID class and navigator.hid', () => {
  ...
  test('onconnect and ondisconnect event handler attributes can be assigned', async ({ sharedPage }) => {
    const result = await sharedPage.evaluate(() => {
      const fn = () => {};
  ...
  test('EventTarget addEventListener/removeEventListener works on navigator.hid', async ({ sharedPage }) => {
    const result = await sharedPage.evaluate(() => {
      let called = false;
