...
test.describe('page-created self-worker (new Worker(location.href))', () => {
  test("page's own worker script runs", async ({ sharedPage, pageUrl }, testInfo) => {
    ...
    await sharedPage.waitForFunction(
      () => (window as unknown as { workerResult?: string }).workerResult !== 'pending',
      { timeout: 10000 },
    );
    const result = await sharedPage.evaluate(
      () => (window as unknown as { workerResult?: string }).workerResult,
    );
    if (result !== 'page-worker-ran') {
      ...
      testInfo.annotations.push({
        type: 'warning',
        description: `known limitation: page self-worker hijacked by shadow-URL interception; page worker script did not run (workerResult=${result})`,
      });
      console.warn(
        `KNOWN LIMITATION: page self-worker hijacked by shadow-URL interception (workerResult=${result})`,
      );
      return;
    }
