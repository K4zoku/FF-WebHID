...
test.describe('Worker polyfill gating', () => {
  ...
  test('blob worker gets no init message even with workerPolyfill on', async ({
    page,
    pageUrl
  }) => {
    ...
    const raw = await page.evaluate<PlainWorkerResult>(async () => {
      const url = URL.createObjectURL(
        new Blob([
          'self.onmessage = (e) => self.postMessage({ gotNull: e.data === null, data: e.data })'
        ])
      )
  ...
  test('live workerPolyfill toggle updates the worker init decision', async ({
    page,
    crossUrl,
    backgroundPage
  }) => {
  ...
  test('early inline worker spawn initializes without waiting for settings', async ({
    page,
    pageUrl,
    backgroundPage,
    servers
    ...
    await page.waitForFunction(
      () => {
        const r = (window as unknown as { tests?: { results?: { race?: { got?: string } } } }).tests
          ?.results?.race
      ...
      () =>
        (
          window as unknown as {
            tests?: {
              results?: { race?: { got: string; hasNavigatorHid?: boolean; state?: string } }
            }
          }
        ).tests?.results?.race
    )
