...
export function startWebhidMock(descriptor: string, vid: number, pid: number): WebhidMockProcess {
  ...
  const proc = spawn(
    ...
    [
      ...
      `0x${pid.toString(16)}`,
      '--descriptor',
      join(projectRoot, 'tests', 'fixtures', 'descriptors', descriptor)
    ],
    {
...
export function installNmManifest(): void {
...
export function installDaemonNmManifest(): void {
  ...
  const manifest = {
    ...
    description: 'WebHID daemon-as-NM-host (e2e: debug daemon direct)',
