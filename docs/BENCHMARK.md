...
**Codebase state at benchmark time**: Worker WS mode was the default `dataPlane: 'ws'` option: a Web Worker owned the WebSocket (without SAB) for off-main-thread WS processing, and forwarded input reports to the page via a 2-hop relay (worker → bridge → page, both hops Transferable). No COOP/COEP injection in either Firefox mode. The polyfill used **zero-copy input report delivery**: DataView created directly on the transferred ArrayBuffer, no intermediate copy.

> **Note on the current codebase**: the benchmark predates the MessagePort direct path. Since then the data worker sends input reports straight to the page through a `MessagePort` transferred once at setup (worker → page, 1 hop, bridge bypassed entirely), and the 2-hop relay is now only the fallback for when the port transfer fails. The WS-vs-NM gap measured here (Worker WS faster) is expected to have widened, not narrowed, because the direct path removes one more context hop and one structured clone per report. Re-benchmarking is on the roadmap; the numbers below are the last full cold-start dataset.

## Post-benchmark developments (2026-07/08)

- **SAB removed entirely** (2026-07). After the ring-buffer alloc-size bug was fixed (exact per-report size from the descriptor instead of an estimate, 64-slot default), SAB was benchmarked against the simpler no-SAB path and lost, while still carrying real ongoing costs (cross-origin isolation requirement, COOP/COEP injection risk, Atomics/ring-buffer complexity). Decision: remove SAB rather than keep it as an opt-in performance mode. See AGENTS.md for the full decision record; this report's "What not to pursue" entry on SAB was updated accordingly.
...
_Note: Run 4 has anomalous idle/memory categorization (idle=0%, mem=426ms). likely a sample classification edge case. Benchmark time (2115.8ms) is consistent with other runs._
...
- Re-add SharedArrayBuffer. Worker WS MessagePort transfer (1 hop direct worker → page, zero-copy) is faster than SAB in cold-start benchmarks and removes COOP/COEP requirement. SAB was removed entirely in 2026-07 after the ring-buffer alloc-size fix: benchmarked against the no-SAB path it lost, while carrying real ongoing costs (cross-origin isolation, COOP/COEP injection risk, Atomics/ring-buffer complexity). See AGENTS.md for the decision record.
