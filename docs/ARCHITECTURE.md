...
- **Data worker → daemon WS/WT:** retry WebSocket/WebTransport (500ms → 5s). On auth-failure (WS close code 4401/4402, WT ready rejection or pre-ready close), halts auto-reconnect and asks bridge for a fresh token via `auth-failed` message; bridge re-opens the device and respawns the worker. Post-ready drops suppress the terminal `closed` signal while a reconnect is pending, so the page plane survives transient blips.
