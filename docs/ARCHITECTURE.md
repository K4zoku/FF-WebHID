...
Firefox 154 applies Local Network Access enforcement to WebSocket connections created in a worker and to page-context WebTransport. Worker-context networking is not categorically exempt. A WebSocket attempt can surface the browser's LNA permission UI, while WebTransport can fail or retry without showing an equivalent prompt when permission has not yet been granted. In practice, granting LNA after a WS attempt can allow WT to work afterward. This is an observed Firefox 154 implementation detail, not a stable browser contract.
...
| ---------------------------------- | --------------------------------- | ----------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `dataPlane`                        | `ws` / `wt` / `nm`                | `wt` when available, otherwise `ws` | Data plane: WT worker (QUIC, pinned self-signed cert, preferred default), WS worker, or NM via bridge                                                                                                                                                                          |
...
| `useWorker`                        | bool                              | `true`                              | WT only: run the data plane in a dedicated worker (default) or in-page on the main thread. WS always uses the worker; NM never does. No longer a UI toggle: WebTransport (in-page) in the Data Plane picker sets `useWorker=false`, WebTransport (worker) sets `true`.         |
...
`daemonAsNmHost` and `hidePageAction` are global-only: neither can be overridden per site. Every other setting (`dataPlane`, `workerSpawnMode`, `useWorker`, `logLevel`, `devicePickerMode`, `workerPolyfillEnabled`, `allowActivationlessRequestDevice`) is per-site overridable from the popup. Changes propagate live through the `storage.onChanged` → `settings.set` path. The bridge re-routes open devices when `dataPlane` or `useWorker` changes, resets spawn-mode state when `workerSpawnMode` changes, re-levels its logger when `logLevel` changes, and re-reads `devicePickerMode` at the next `requestDevice()` call. `workerPolyfillEnabled` affects future page-created workers after reload.
...
`useWorker: false` (WT only) skips worker spawn entirely: the bridge sends `dataPlaneConnect` over the page port and the polyfill hosts the WT connection in-page (see the WT data plane section). The popup shows `workerSpawnMode` only when a worker will actually spawn (data plane `wt` or `ws`).
