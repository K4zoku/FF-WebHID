...
| `Arc::from(&frame[6..])` in WS/WT batch dispatch (batching.rs) | Zero-copy slice for spawn_blocking                                                                                                                      |
...
- **Data worker → daemon WS/WT:** retry WebSocket/WebTransport (500ms → 5s). On auth-failure (WS close code 4401/4402, WT ready rejection or pre-ready close), halts auto-reconnect and asks bridge for a fresh token via `auth-failed` message; the bridge re-attaches the plane with a live session token (never a new daemon session) and respawns the worker. Post-ready drops suppress the terminal `closed` signal while a reconnect is pending, so the page plane survives transient blips.
