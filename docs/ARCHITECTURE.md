...
| `utils/webtransport.js`           | `createWtTransport` factory: WT connect over `serverCertificateHashes`-pinned QUIC; one `onBinary` per daemon stream; auth-failure detection via ready rejection or early close; no auto-reconnect (v1) |
...
- Per-message QUIC bidirectional streams: each daemon-to-page batch flush and each control response is written to its own stream and closed (FIN is the message boundary), so no new framing/reassembly layer is needed; the JS transport delivers one `onBinary` per stream.
