...
`daemonAsNmHost` and `hidePageAction` are global-only. Every other setting can be overridden per-site via the popup. Settings use per-key format in `browser.storage.local`: global keys are `settings :: <name>`, site overrides are `settings :: <origin> :: <name>`. Device info + origin allowlists are stored in IndexedDB (`webhid-store`).
...
- `tests/e2e/wt.spec.ts` (10 tests): WebTransport data plane (WT spawn on open, input/send/feature over WT, no NM double-delivery while in WT mode, live ws→wt switch on an open device, cert-generation rotation + drain)
