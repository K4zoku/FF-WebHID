...
| `workerSpawnMode`       | `shadow` / `blob`                 | `shadow` (MV3) / `blob` (MV2) | How the data worker is spawned in page context. `shadow` = `new Worker(location.href)` with webRequest interception (Shadow URL). `blob` = blob URL from the extension's worker bundle, requires CSP rewrite. Background pre-flights the page CSP (`getCspInfo`) and falls back: shadow blocked → blob, blob blocked on MV3 → NM. Hidden in the UI when `useWorker` is off or the data plane is `nm`. |
| `useWorker`             | bool                              | `true`                        | WT only: run the data plane in a dedicated worker (default) or in-page on the main thread. WS always uses the worker; NM never does. Hidden in the UI unless `dataPlane` is `wt`.                                                                                                                                                                                                                     |
