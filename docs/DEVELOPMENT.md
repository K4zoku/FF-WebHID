...
| ----------------------- | --------------------------------- | ----------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `dataPlane`             | `ws` / `nm`                       | `ws`                          | Data plane: WS worker (MessagePort direct to page) or NM via bridge. Control ops are always NM.                                                                                                                                                                                                                                   |
...
- A mock device whose _top-level_ collection matches `crates/webhid-daemon/src/blocklist.rs` (Generic Desktop Mouse/Keyboard/Keypad/System Control, FIDO 0xF1D0) is hidden from enumeration, so the grant picker shows nothing. The `vendor.bin` fixture's first top-level is a Joystick (0x01/0x04), which is not blocked. Verify a fixture's top-level usage against this blocklist before giving it a primary role.
