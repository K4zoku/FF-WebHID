...
- Report-level blocking (`report-blocking` cargo feature, on by default) drops input reports and rejects output/feature writes whose report ID is declared by any collection matching `crates/webhid-daemon/src/blocklist.rs` (Generic Desktop Mouse/Keyboard/Keypad/System Control, FIDO 0xF1D0); the device itself stays enumerable, matching Chromium. The `vendor.bin` fixture (Joystick 0x01/0x04 + vendor 0xFF1C report 1) and `gamepad.bin` (Joystick 0x01/0x04, unnumbered) carry no blocked usages, so they are unaffected in default builds; a `mouse.bin`/`keyboard.bin` fixture would have its reports dropped (mouse reports sit in a Physical child whose 0x09 usage does not match, so only the keyboard fixture is actually blocked). Verify a fixture's usages against this blocklist before giving it a primary role in a default-features build.
