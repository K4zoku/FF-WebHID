...
End-to-end tests spawn the debug daemon + `webhid-mock` (a virtual HID device creator: `/dev/uhid` on Linux, `IOHIDUserDevice` on macOS) as subprocesses and drive `navigator.hid` through a test page. Uses a Switch Pro gamepad descriptor fixture.
...
│   ├── tests/e2e/           E2E specs (webhid-mock + daemon subprocess)
...
├── scripts/
│   └── gen-descriptors.mjs  Generates sample HID report descriptors for webhid-mock tests
