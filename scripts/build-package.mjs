...
function installNmManifests(base, nmBin, daemonBin) {
  for (const b of ['mozilla', 'librewolf', 'waterfox']) {
    ...
    template(join(MANIFESTS, 'webhid.forwarder_nm_host.json'),
      join(d, 'webhid.forwarder_nm_host.json'), { '{{NM_BIN}}': nmBin });
    template(join(MANIFESTS, 'webhid.daemon_nm_host.json'),
      join(d, 'webhid.daemon_nm_host.json'), { '{{DAEMON_BIN}}': daemonBin });
  }
}
...
function installSystemd(base, daemonBin) {
  ...
  mkdirSync(d, { recursive: true });
  template(join(MANIFESTS, 'webhid-daemon.service'),
    join(d, 'webhid-daemon.service'), { '{{DAEMON_BIN}}': daemonBin });
}
...
function buildDeb(ver, arch) {
  ...
  writeFileSync(join(DEBIAN, 'postinst'), [
    '#!/bin/sh',
  ...
  writeFileSync(join(DEBIAN, 'prerm'), [
    '#!/bin/sh',
