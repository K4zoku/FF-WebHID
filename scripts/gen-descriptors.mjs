...
function collection(t = 1) { return Buffer.from([0xA1, t]); }
function endCollection() { return Buffer.from([0xC0]); }
function reportId(r) { return Buffer.from([0x85, r]); }
function reportSize(s) { return Buffer.from([0x75, s]); }
function reportCount(c) { return Buffer.from([0x95, c]); }
function inputData(flags = 0x02) { return Buffer.from([0x81, flags]); }
function outputData(flags = 0x02) { return Buffer.from([0x91, flags]); }
...
function unitExponent(e) { return Buffer.from([0x55, e & 0xFF]); }
function unitBytes(b) { return Buffer.from([0x65, b.length, ...b]); }

function usageMinimum(u) {
  return u <= 0xFF
    ? Buffer.from([0x18, u])
    : Buffer.from([0x19, u & 0xFF, (u >> 8) & 0xFF]);
}

function usageMaximum(u) {
  return u <= 0xFF
    ? Buffer.from([0x28, u])
    : Buffer.from([0x29, u & 0xFF, (u >> 8) & 0xFF]);
}
...
function keyboardDescriptor() {
  return cat(
    usagePage(0x01), usage(0x06), collection(),
    reportSize(1), reportCount(8),
    usagePage(0x07),
    usage(0xE0), usage(0xE1), usage(0xE2), usage(0xE3),
    usage(0xE4), usage(0xE5), usage(0xE6), usage(0xE7),
    inputData(),
    reportSize(8), reportCount(1), inputData(),
    reportSize(8), reportCount(6),
    usage(0x00), inputData(),
    endCollection(),
  );
}
...
function edgeEmpty() { return Buffer.alloc(0); }

function edgeSingleByte() { return Buffer.from([0xFF]); }

function edgeTruncatedInput() { return Buffer.from([0x81]); }

function edgeTruncatedLongItem() { return Buffer.from([0xFE, 0xFF]); }

function edgeUnclosedCollection() {
  return cat(
    usagePage(0x01), usage(0x02), collection(),
    reportSize(8), reportCount(1),
    usagePage(0x09), usage(1),
    inputData(),
  );
}

function edgeExtraEndCollection() { return Buffer.from([0xC0]); }
...
function edgeMultipleReportIds() {
  ...
  for (const rid of [1, 2, 3]) {
    reportChunks.push(
      reportId(rid),
      reportSize(8), reportCount(4),
      usagePage(0x01), usage(0x30), usage(0x31), usage(0x32), usage(0x35),
      inputData(),
    );
  }
...
function edgeReportSizeMax() {
  return cat(
    usagePage(0x01), usage(0x02), collection(),
    reportSize(32), reportCount(0xFF),
    usagePage(0x09), usage(1),
    logicalMin(0), logicalMax(0xFFFFFFFF),
    inputData(),
    endCollection(),
  );
}
...
function edgeVendorExtendedUsage() {
  return cat(
    ...
    collection(),
    reportSize(8), reportCount(16),
    usageMinimum(0x01), usageMaximum(0x10),
    logicalMin(0), logicalMax(255),
    inputData(0x00),
    endCollection(),
  );
...
function edgeVariableAfterArray() {
  return cat(
    ...
    reportSize(8), reportCount(4),
    usagePage(0x07),
    usageMinimum(0x00), usageMaximum(0x03),
    logicalMin(0), logicalMax(255),
    inputData(0x00),
    reportSize(8), reportCount(2),
    usagePage(0x01),
